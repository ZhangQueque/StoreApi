...
namespace Store.Service.Users
{
    public class UserRepository:RepositoryBase<UserInfo,int>, IUserRepository
    {
        ...
        {
            var user = await context.Set<UserInfo>().FirstOrDefaultAsync(m => m.Email == email && m.Password == password);
        ...
        {
            return await context.Set<UserInfo>().FirstOrDefaultAsync(m => m.Email == email)!=null;
        }

        public async Task<bool> IsExistPhoneAccountAsync(string phone)
        {
            return await context.Set<UserInfo>().FirstOrDefaultAsync(m => m.Phone == phone) != null;
        }

        public async Task<UserInfo> PhoneLoginAsync(string phone)
        {
            var user = await context.Set<UserInfo>().FirstOrDefaultAsync(m => m.Phone == phone);
